...
class AccountUpdateClaimantDetailsV7ImplTest {
  private AccountUpdateClaimantDetailsV7Impl cut;
  ...
  void setup() {
    ...
    cut = new AccountUpdateClaimantDetailsV7Impl(claimantService, dataMapper, updateEmailImpl, updateNinoImpl);
  }
  ...
  void testUpdateAccountClaimantDetails() {
    ...
    var req = mock(V4UpdateClaimantDetailsRequest.class);
    ...
    var rc = V4UpdateClaimantDetailsRequest.ResearchContactEnum.NO;
    when(req.getResearchContact()).thenReturn(rc);
    ...
    order.verify(dataMapper).mapToV7AccountDetails(claimantCaptor.capture());
