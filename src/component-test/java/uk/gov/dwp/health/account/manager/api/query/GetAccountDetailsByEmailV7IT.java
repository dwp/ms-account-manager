...
import static uk.gov.dwp.health.account.manager.utils.UrlBuilderUtil.getAccountByEmailV7Url;
...
class GetAccountDetailsByEmailV7IT extends ApiTest {
  ...
  void shouldReturn200StatusCodeAndAccountDetailsWhenRetrieveAccountByEmail() {
    Response response = getRequest(getAccountByEmailV7Url(createAccountRequest.getEmail()));
  ...
  void shouldReturn400StatusCodeWhenRetrieveAccountWithInvalidEmail() {
    Response response = getRequest(getAccountByEmailV7Url("not-a-valid-email"));
